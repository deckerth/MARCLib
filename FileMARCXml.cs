...
namespace MARC
{
	...
	public class FileMARCXml : IEnumerator, IEnumerable
	{
		...
		private readonly List<XElement> _rawSource;
        ...
        {
            get { return _nextRecordPosition; }
        }
		...
		public int Add(XDocument source)
		{
            ...
            bool nextRecordPositionFound = false;
            _nextRecordPosition = 0;

            //BVBR Format
            foreach (XElement searchResult in source.Elements().Where(e => e.Name.LocalName == "searchRetrieveResponse"))
            {
                foreach (XElement noOfRecords in searchResult.Elements().Where(e => e.Name.LocalName == "numberOfRecords"))
                {
                    _numberOfEntries = Convert.ToInt32(noOfRecords.FirstNode.ToString());
                }
                foreach (XElement collection in searchResult.Elements().Where(e => e.Name.LocalName == "records")) 
                {
                    foreach (XElement rec in collection.Elements().Where(e => e.Name.LocalName == "record"))
                    {
                        foreach (XElement recordData in rec.Elements().Where(e => e.Name.LocalName == "recordData"))
                        {
                            foreach (XElement record in recordData.Elements().Where(e => e.Name.LocalName == "record"))
                            {
                                _rawSource.Add(record);
                                addCount++;
                            }
                        }
                        foreach (XElement recordPosition in rec.Elements().Where(e => e.Name.LocalName == "recordPosition"))
                        {
                            lastRecordPosition = Convert.ToInt32(recordPosition.FirstNode.ToString());
                        }
                    }
                }
                foreach (XElement nextRecPos in searchResult.Elements().Where(e => e.Name.LocalName == "nextRecordPosition"))
                {
                    _nextRecordPosition = Convert.ToInt32(nextRecPos.FirstNode.ToString());
                    nextRecordPositionFound = true;
                }
            }
            ...
            //Some libraries do not send the field nextRecordPosition
            if (!nextRecordPositionFound && (lastRecordPosition > 0) && (lastRecordPosition < _numberOfEntries))
            {
                _nextRecordPosition = lastRecordPosition + 1;
            }
		...
		{
			_position++;
			return (_position < _rawSource.Count);
		}
